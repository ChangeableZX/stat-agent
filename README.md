...
以下分析类型**不在支持范围内**，Agent 会明确拒绝而非强行映射到已有方法：

| 不支持的分析 | 原因 |
|---|---|
| 时间序列预测（ARIMA、趋势、季节分解） | 时间序列预测 ≠ 横截面线性回归，强行 `regression` 在统计上是错误的 |
| Logistic 回归、Ridge、Lasso、多项式回归 | 回归模块只支持 OLS 线性回归 |
| 聚类分析、PCA、因子分析 | 无对应工具 |
| 生存分析、贝叶斯分析 | 无对应工具 |

如需上述分析，需在 `tools.py` 和 `decision.py` 中扩展对应的 intent 和工具链。

### 意图路由行为

**意图模糊**（如"帮我看看这数据"）：Agent 不会自动选择意图，会主动列出支持的 6 类分析引导用户选择。这是预期设计，不是 bug。

**字段缺失**（如"比较两组"但未说明哪两列）：`make_analysis_plan` 会返回缺少字段的错误，Agent 会反问用户补充信息。

**歧义意图**（如"性别和收入有没有关系"）：当问题中一个变量是二值分类（男/女）、另一个是连续变量时，Agent 优先路由到 `compare_two_groups`（比较两组均值），仅当用户明确要求"相关系数"时才选 `correlation`。两种选择在统计上均合理，选择会在回复中说明理由。

### 多组数据比较特定两组

当数据列含 3 个及以上组（如 A/B/C），但用户只要求"比较 A 和 B"时，Agent 会向 `run_independent_ttest`、`run_welch_ttest`、`run_mannwhitney`、`check_variance_equality` 等工具传入 `group_values=["A","B"]` 参数，自动过滤到指定两组。如果 Agent 未正确使用该参数，工具会报错提示"找到 N 个组，需要恰好 2 个"。

### 列名错误或不存在

`make_analysis_plan` 不做列名校验（数据尚未加载）。`load_data` 完成后会立即比对计划中的变量名与实际列名，如有不匹配，在返回结果中注入 `plan_variable_warning` 字段，Agent 应在此时停止并告知用户正确列名，不会继续调用后续统计工具。
...
- 图表保存在本地 `output/` 目录，无自动清理机制。
- 暂不支持多用户、登录、会话持久化或数据库存储。
- 单次对话最多 15 轮工具调用循环；复杂分析链（多元回归 + 全套诊断）偶尔会接近上限。
